...
from invenio.webauthorprofile_corefunctions import get_pubs, get_person_names_dicts, \
    ...
    get_total_downloads, get_cited_by_list, get_kwtuples, get_fieldtuples, \
    get_veryfy_my_pubs_list_link, get_hepnames_data, get_self_pubs, get_collabtuples, \
    get_person_oldest_date, expire_all_cache_for_person
...
class WebAuthorPages(WebInterfaceDirectory):
    ...
    def create_authorpage_websearch(self, req, form, person_id, ln='en', expire_cache=False):
        ...
        if form.has_key('jsondata'):
            ...
            json_response['boxes_info'].update({'combined_papers': {'status':(beval[3] and beval[12]), 'html_content': webauthorprofile_templates.tmpl_papers_with_self_papers_box(pubs, selfpubs, bibauthorid_data, totaldownloads, ln, add_box=False, loading=not beval[3])}})
            # json_response['boxes_info'].update({'papers': {'status':beval[3], 'html_content': webauthorprofile_templates.tmpl_papers_box(pubs, bibauthorid_data, totaldownloads, ln, add_box=False, loading=not beval[3])}})
            json_response['boxes_info'].update({'selfpapers': {'status':beval[13], 'html_content': webauthorprofile_templates.tmpl_self_papers_box(selfpubs, bibauthorid_data, totaldownloads, ln, add_box=False, loading=not beval[13])}})
            ...
            json_response['boxes_info'].update({'fieldcodes': {'status':beval[5], 'html_content': webauthorprofile_templates.tmpl_fieldcode_box(fieldtuples, bibauthorid_data, ln, add_box=False, loading=not beval[5])}})
            ...
            json_response['boxes_info'].update({'coauthors': {'status':beval[6], 'html_content': webauthorprofile_templates.tmpl_coauthor_box(bibauthorid_data, coauthors, ln, add_box=False, loading=not beval[6])}})
            ...
            json_response['boxes_info'].update({'pubs_graph': {'status':beval[11], 'html_content': webauthorprofile_templates.tmpl_graph_box(pubs_per_year, authorname, ln, add_box=False, loading=not beval[11])}})
            json_response['boxes_info'].update({'hepdata': {'status':beval[12], 'html_content':webauthorprofile_templates.tmpl_hepnames(hepdict, ln, add_box=False, loading=not beval[12])}})
            json_response['boxes_info'].update({'collaborations': {'status':beval[14], 'html_content': webauthorprofile_templates.tmpl_collab_box(collab, bibauthorid_data, ln, add_box=False, loading=not beval[14])}})
            ...
            req.write(webauthorprofile_templates.tmpl_author_page(pubs, \
                                            ...
                                            author_aff_pubs, citedbylist, kwtuples, \
