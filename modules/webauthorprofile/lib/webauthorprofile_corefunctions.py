...
def get_cited_by_list(person_id):
    '''
    Returns a list of citations for the giver person id.
    @param person_id: int person id
    @return [
            [[ [recid, [cits] ],
            bool]
...
def _get_institute_pubs_dict(recids, names_list):
    ...
    a = [pickle.loads(p.strip()) for p in a.strip().split('!---THEDELIMITER---!') if p]
    affdict = {}
    for rec, affs in a:
        keys = affs.keys()
...
def _get_pubs_per_year_dictionary(pubyearslist):
    ...
    if not yearsdict:
        return res
    start = min(yearsdict.keys()) - 1
    end = datetime.now().year + 2
    if end - start < 6:
        start = end - 6
    for year in range(start, end):
        try:
            res.append((year, yearsdict[year]))
        except KeyError:
            res.append((year, 0))
    return res
...
def _get_hepnames_data_bai(bibauthorid_data, person_id):
    ...
    return _get_hepnames_data_dict(bibauthorid_data, person_id, cid, hepRecord)

def _get_hepnames_data_dict(bibauthorid_data, person_id, cid, hepRecord):
    hepdict = {}
    hepdict['cid'] = cid
