        ...
        def cache_filler():
            ...
            for row in res:
                ...
                if object_name == 'citationdict':
                    ...
                    # Citation counts
                    alldicts['citationdict_counts'] = [(recid, len(cites)) for recid, cites in object_value_dict.iteritems()]
                    alldicts['citationdict_counts'].sort(key=itemgetter(1), reverse=True)
                    ...
                    alldicts['selfcites_counts'] = [(recid, cites - selfcites.get(recid, 0)) for recid, cites in alldicts['citationdict_counts']]
...
def get_records_with_num_cites(numstr, allrecs=intbitset([])):
    """Return an intbitset of record IDs that are cited X times,
       ...
       be 10,0->100 etc
    """
    cache_cited_by_dictionary_counts = get_citation_dict("citationdict_counts")
...
def calculate_co_cited_with_list(record_id, sort_order="d"):
    ...
    for cit_id in citation_list:
        ...
        for ref_id in reference_list:
            if not ref_id in result_intermediate:
