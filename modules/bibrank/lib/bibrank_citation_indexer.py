...
def print_cites_diff(recids, refs_diff, cites_diff):
    ...
    """
    sorted_refs_balance = sorted(zip(recids, refs_diff),
                                 key=lambda x: x[1],
                                 reverse=True)
    for recid, record_refs_diff in list(sorted_refs_balance)[:20]:
        write_message('%s balance %s refs' % (recid, record_refs_diff))

    sorted_cites_balance = sorted(zip(recids, cites_diff),
                                  key=lambda x: x[1],
                                  reverse=True)
    for recid, record_cites_diff in list(sorted_cites_balance)[:20]:
        write_message('%s balance %s cites' % (recid, record_cites_diff))
