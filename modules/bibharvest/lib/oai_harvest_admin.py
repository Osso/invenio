...
def getheader():
    ...
    return bibharvest_templates.tmpl_meta_headers()
...
def perform_request_index(ln=CFG_SITE_LANG):
    ...
    for (oai_src_id, oai_src_baseurl, oai_src_prefix, \
         dummy1, oai_src_comment, dummy2, oai_src_name, \
         oai_src_lastrun, oai_src_frequency, oai_src_post, \
         dummy3, dummy4) in oai_src:
        ...
        if not oai_src_lastrun:
            oai_src_lastrun = bibharvest_templates.tmpl_print_warning(ln, \
                                                                      _("Never harvested"), \
                                                                      prefix="")
        else: 
...
def perform_request_editsource(oai_src_id=None, oai_src_name='',
    ...
    if confirm == -1:
        ...
        if oai_src_config:
            oai_src_args['c_cfg-file'] = oai_src_config
        if oai_src_bibfilter:
            oai_src_args['f_filter-file'] = oai_src_bibfilter
    elif confirm == 1:
        warnings = []
        if not oai_src_name:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please enter a name for the source."))
        if not oai_src_prefix:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please enter a meta-data prefix."))
        if not oai_src_frequency:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please choose a frequency of harvesting"))
    ...
    if show_form:
        text = get_oai_source_form(ln=ln,
                                   ...
                                   editing_mode=True)
        output += bibharvest_templates.tmpl_form_vertical(action="editsource#1",
                                                          text=text,
                                                          button="Modify",
...
def perform_request_addsource(oai_src_name=None, oai_src_baseurl='',
    ...
    if confirm <= -1:
        ...
        text += bibharvest_templates.tmpl_admin_w200_text_placeholder(ln=ln,
        ...
        if not oai_src_name:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please enter a name for the source."))
        if not oai_src_prefix:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please enter a metadata prefix."))
        if not oai_src_frequency:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please choose a frequency of harvesting"))
        if not oai_src_lastrun:
            warnings.append(bibharvest_templates.tmpl_print_warning(ln, "Please choose the harvesting starting date"))
    ...
    if show_form:
        text = get_oai_source_form(ln=ln,
                                   ...
                                   editing_mode=False)
        output += bibharvest_templates.tmpl_form_vertical(action="addsource#1",
                                                          text=text,
...
def get_oai_source_form(ln, oai_src_baseurl, oai_src_name, oai_src_prefix,
    ...
    table_first_col += bibharvest_templates.tmpl_admin_w200_text(ln=ln,
                                                     title="Source name",
                                                     name="oai_src_name",
                                                     value=oai_src_name,
                                                     suffix=" e.g. arxiv<br />")
    ...
    if metadatas:
        ...
        table_first_col += bibharvest_templates.tmpl_admin_w200_select(ln=ln,
    ...
    table_first_col += bibharvest_templates.tmpl_admin_w200_textarea(ln=ln,
    ...
    table_first_col += bibharvest_templates.tmpl_admin_checkbox_arguments(ln=ln, title="",
                                                                          name="oai_src_post",
                                                                          values=post_values,
                                                                          labels=post_labels,
                                                                          states=post_states,
    ...
    if sets:
        ...
        sets_states = [ ((set[0] in oai_src_sets and 1) or 0) \
                        for set in sets]
        table_second_col += bibharvest_templates.tmpl_admin_checkboxes(ln=ln,
                                                           title="",
                                                           name="oai_src_sets",
                                                           values=sets_specs,
                                                           labels=sets_labels,
        ...
        table_second_col += bibharvest_templates.tmpl_admin_w200_text(ln=ln,
                                                          title="Sets",
                                                          name='oai_src_sets',
                                                          value=' '.join(oai_src_sets),
                                                          suffix=" e.g. physics<br />")

    output += bibharvest_templates.tmpl_output_table(data=([table_first_col, "_SEPARATOR_", table_second_col],), \
    ...
    for mode_value, mode_label, mode_arguments in modes:
        for arg_dict in mode_arguments:
            ...
            if arg_dict['required'] and not oai_src_args[name]:
                warning_list.append(bibharvest_templates.tmpl_print_warning(ln, \
