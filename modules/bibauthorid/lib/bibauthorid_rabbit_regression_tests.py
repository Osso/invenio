...
class BibAuthorIDRabbitTestCase(InvenioTestCase):
    ...
    def setUp(self):
        ...
        self.author_name = 'TestName TestSurname'  # The original name.

        #A one-letter change to the original name.
        self.slightly_modified_author_name = 'TestName TestSorname'

        #A rather large change of the original name.
        self.heavily_modified_name = 'TestSurname TestName'
        ...
        self.co_authors_names = ['Coauthor SomeCoauthor',
                                ...
                                'Queen Elizabeth',
                                'John SomeBody']
        self.slightly_mod_co_authors_names = ['Coauthor SomeCauthor',
                                             'SomeCoauthor DifferntCoauthor',
                                             'Queen Elizebeth',
                                             'John SameBody']
        self.heavily_mod_co_authors_names = ['SomeCoauthor Coauthor',
                                            'DifferentCoauthor SomeCoauthor',
                                            'Elizabeth Queen',
                                            'Somebody John']
...
class OneAuthorRabbitTestCase(BibAuthorIDRabbitTestCase):
    ...
    def setUp(self):
        ...
        self.main_marcxml_record = get_new_marc_for_test(author_name=self.author_name,
                                                         ext_id=self.ext_id)
    ...
    def test_rabbit_one_author_only(self):
        ...
        def test_rabbit_heavily_modify_author():
            ...
            different thanthe original.. After the run of rabbit, the name in
...
class CoauthorsRabbitTestCase(BibAuthorIDRabbitTestCase):
    ...
    def test_rabbit_with_coauthors(self):
        ...
        def test_rabbit_slightly_modify_coauthors():
            ...
            previous_bibrefs_of_coauthors = deepcopy(self.current_bibref_values_of_coauthors)
            for index, _ in enumerate(self.current_bibref_values_of_coauthors):
                self.current_bibref_values_of_coauthors[index] = get_bibref_value_for_name(self.slightly_mod_co_authors_names[index])
            number_of_personids_after = get_count_of_pids()
            self.assertTrue(person_in_aidpersonidpapers(self.author_name, self.main_bibrec))
            self.assertEquals(previous_bibref_value_of_author,
                              self.current_bibref_value_of_author)
...
class MatchableNameRabbitTestCase(BibAuthorIDRabbitTestCase):
    ...
    def setUp(self):
        ...
        main_marcxml_record = get_new_marc_for_test(author_name=self.author_name)
    ...
    def test_rabbit_matchable_name(self, mocked_func1, mocked_func2):
        ...
        second_marcxml_record = get_new_marc_for_test(author_name=self.heavily_modified_name)
...
class MnamesCacheConsistencyTestCase(BibAuthorIDRabbitTestCase):
    ...
    @patch('invenio.bibauthorid_rabbit.find_pids_by_exact_name')
