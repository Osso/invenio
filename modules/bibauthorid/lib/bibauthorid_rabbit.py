...
from invenio.bibauthorid_backinterface import get_authors_by_name as find_pids_by_exact_name,  \
...
from invenio.bibauthorid_logutils import update_status, update_status_final
from invenio.bibtask import task_update_progress
from datetime import datetime
from invenio.dbquery import run_sql
from invenio.bibauthorid_logutils import bibauthorid_print
from invenio.bibauthorid_logutils import bibauthorid_verbose
...
def rabbit(bibrecs=None, check_invalid_papers=False,
    ...
    if verbose:
        bibauthorid_verbose(True)

    if not bibrecs:
        bibauthorid_print(True, "RABBIT %s running on %s \n" % (str(now()), 'All records'))
    else:
        bibauthorid_print(True, "RABBIT %s running on %s \n" % (str(now()), str(bibrecs)))
    ...
    def find_pids_by_exact_name_with_cache(matchable_name):
        try:
            matched_pids = [M_NAME_PIDS_CACHE[matchable_name]]
        except KeyError:
            matched_pids = find_pids_by_exact_name(matchable_name,
    ...
    for idx, rec in enumerate(bibrecs):
        ...
        bibauthorid_print(True, "Considering %s" % str(rec))
        ...
        if idx % 1000 == 0:
            ...
            update_status(float(idx) / len(bibrecs), "%d/%d" % (idx, len(bibrecs)))
        ...
        for sig in not_matched:
            ...
            matched_pids = find_pids_by_exact_name_with_cache(matchable_name)
        ...
        bibauthorid_print(True, 'Finished with %s' % str(rec))
