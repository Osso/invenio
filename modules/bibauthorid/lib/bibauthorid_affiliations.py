...
def get_personids(recid):
    pids_rows = run_sql("""SELECT personid, name
                           FROM aidPERSONIDPAPERS
                           WHERE bibrec = %s""", [recid])
    pids = {}
    for personid, name in pids_rows:
        pids[name] = personid
    return pids
...
def store_aff(cur_aff, aff):
    ...
    for pid, aff_info in aff.iteritems():
        ...
        if aff_info != cur_aff_info:
            write_message('Affiliation changed for %s, new info %s @ %s in %s' % (pid, aff_info['aff'], aff_info['last_occurence'].strftime("%Y-%m-%d %H:%M:%S"), aff_info['last_recid']), verbose=0)
            if cur_aff_info:
