...
def getHoldingPenData(req, elementId):
    ...
    if len(elements) == 2:
        ...
        for i in range(0, numberOfPages):
            pages += [
            {
            "url": "%s/admin/oaiharvest/oaiharvestadmin.py/get_entries_fragment?year=%s&month=%s&day=%s&start=%i&limit=%i&filter=%s" % (CFG_SITE_URL, nodeYear, nodeMonth, nodeDay, i * resultsPerPage, resultsPerPage, urlFilter),
            "selector": False,
            "type": "ajax",
            }]
